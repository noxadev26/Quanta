...
import { getFirestore, collection, query, orderBy, getDocs, doc, getDoc, setDoc, addDoc, serverTimestamp, updateDoc, arrayUnion, arrayRemove, deleteDoc } from 'firebase/firestore'
import './App.css'
...
// UID ADMIN KAMU - GANTI DENGAN UID KAMU SENDIRI
  ...
  useEffect(() => {
    onAuthStateChanged(auth, async (u) => {
      if(u) {
        ...
        setUserData(data)
        setIsAdmin(u.uid === ADMIN_UID) // CEK APAKAH ADMIN
        setPage('beranda')
        loadPosts()
        if(u.uid === ADMIN_UID) loadUsers() // KALAU ADMIN LOAD SEMUA USER
      } else {
        setUser(null)
        setPage('login')
      }
      ...
      {users.map(u => (
        <div className="admin-card" key={u.id}>
          ...
          <div className="admin-actions">
            ...
            <button className="btn-delete" onClick={() => hapusUser(u.id)}>Hapus</button>
          </div>
        </div>
      ))}
    </div>
...
// KOMPONEN LOGIN GLASSMORPHIC - SAMA KAYA SEBELUMNYA
    ...
    <div className="auth-wrapper">
      <div className="glass-box">
        ...
        <div className="input-password">
          <input placeholder="Password" type={showPass ? "text" : "password"} value={pass} onChange={e => setPass(e.target.value)} />
...
// KOMPONEN DAFTAR - SAMA KAYA SEBELUMNYA
  ...
  return (
    <div className="auth-wrapper">
      <div className="glass-box">
        ...
        <div className="input-password">
          <input placeholder="Password *" type={showPass ? "text" : "password"} value={form.pass} onChange={e => setForm({...form, pass: e.target.value})} />
          <span onClick={() => setShowPass(!showPass)}>{showPass ? '🙈' : '👁️'}</span>
        </div>
        <input placeholder="Umur *" type="number" value={form.umur} onChange={e => setForm({...form, umur: e.target.value})} />
        <input placeholder="Bidang Studi *" value={form.bidang} onChange={e => setForm({...form, bidang: e.target.value})} />
        <input placeholder="Domisili (Opsional)" value={form.domisili} onChange={e => setForm({...form, domisili: e.target.value})} />
        <select value={form.kelamin} onChange={e => setForm({...form, kelamin: e.target.value})} className="input-select">
...
function PostingPage({ user, userData, setPage }) {
  ...
  return(<div className="content"><div className="posting-box"><h2>Buat Postingan</h2><select value={tag} onChange={e => setTag(e.target.value)} className="input-select"><option>🧠 Edukasi</option><option>👥 Sosial</option></select><input placeholder="Judul Postingan" value={judul} onChange={e => setJudul(e.target.value)} /><textarea rows="5" placeholder="Isi Postingan..." value={text} onChange={e => setText(e.target.value)}></textarea><button className="btn-primary" onClick={posting}>Posting Sekarang</button></div></div>)
}
